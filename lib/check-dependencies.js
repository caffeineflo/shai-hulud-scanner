...
function parsePnpmLock(content) {
  ...
  try {
    ...
    for (const line of lines) {
      // Look for entries like:   posthog-node@4.18.0:
      //                            @babel/code-frame@7.24.1:
      // Pattern matches: leading whitespace, package-name@version:
...
function checkDependencies(packageJsonContent, lockFiles, maliciousPackages) {
  ...
  try {
    ...
    // First, get all locked versions if available (they take precedence)
    let lockedVersions = {};
    if (lockFiles) {
    ...
    for (const [depName, depVersion] of Object.entries(allDeps)) {
      // Skip if we have a lock file version - we'll check that instead for accuracy
      if (lockedVersions[depName]) {
        continue;
      }
    ...
    // Note: lockedVersions was already parsed earlier and takes precedence
